...
train_images_reshaped = np.expand_dims(train_images,axis=3) # reshape to mum_train_images X height X width X channels, where channels = 1
test_images_reshaped = np.expand_dims(test_images,axis=3)   # reshape
...
model = keras.Sequential([
    ...
    keras.layers.Conv2D(64, (3, 3), strides=1, padding='same', input_shape=(14, 14, 32)),
    keras.layers.BatchNormalization(),
    keras.layers.Activation(keras.activations.relu),
    keras.layers.Conv2D(64, (3, 3), strides=1, padding='same', input_shape=(14, 14, 64)),
    keras.layers.BatchNormalization(),
    keras.layers.Activation(keras.activations.relu),
    keras.layers.MaxPooling2D((2, 2), padding='valid'),
    keras.layers.Dropout(0.3),
    keras.layers.Conv2D(128, (3, 3), strides=1, padding='same', input_shape=(7, 7, 64)),
    keras.layers.BatchNormalization(),
    keras.layers.Activation(keras.activations.relu),
    keras.layers.MaxPooling2D((2, 2), padding='valid'),
    keras.layers.Dropout(0.4),
    keras.layers.Flatten(input_shape=(3, 3, 128)),
